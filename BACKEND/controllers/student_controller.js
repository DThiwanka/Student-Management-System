...
const studentRegister = async (req, res) => {
    try {
        ...
        if (existingStudent) {
            res.send({ message: 'NIC Number already exists' });
        }
        else {
...
const updateStudent = async (req, res) => {
    try {
        if (req.body.password) {
            const salt = await bcrypt.genSalt(10);
            req.body.password = await bcrypt.hash(req.body.password, salt); // Corrected
        }
